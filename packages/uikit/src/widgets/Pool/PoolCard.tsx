...
export function PoolCard<T>({ pool, cardContent, aprRow, isStaked, cardFooter, tokenPairImage }: PoolCardPropsType<T>) {
  ...
  const isCakePool = earningToken?.symbol === "GHOSTTY" && stakingToken?.symbol === "GHOSTTY";
        ...
        {totalStaked && totalStaked.gte(0) ? (
          <>
            <PoolCardHeaderTitle
              ...
              subTitle={
                isCakePool ? t("Earn GHOSTTY, stake GHOSTTY") : t("Stake %symbol%", { symbol: stakingToken?.symbol || "" })
              }
            />
            {tokenPairImage}
          </>
