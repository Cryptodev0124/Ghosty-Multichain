...
const BannerPlaceHolder = styled.div<{ walletConnected: boolean }>`
  ...
  ${({ theme }) => theme.mediaQueries.sm} {
    margin-top: ${({ walletConnected }) => (walletConnected ? '190px' : '14px')};
    // margin-bottom: 30px;
  }
  ${({ theme }) => theme.mediaQueries.md} {
    margin-top: ${({ walletConnected }) => (walletConnected ? '90px' : '14px')};
    // margin-bottom: ${({ walletConnected }) => (walletConnected ? '40px' : '30px')};
  }
  ${({ theme }) => theme.mediaQueries.lg},${({ theme }) => theme.mediaQueries.md} {
    padding-top: 0;
    margin-top: ${({ walletConnected }) => (walletConnected ? '60px' : '14px')};
...
const MultipleBanner: React.FC<React.PropsWithChildren> = () => {
  ...
  return (
    <BannerPlaceHolder walletConnected={Boolean(account) && chainId === ChainId.BSC || chainId === ChainId.ETHEREUM}>
      <StyledSwiper
